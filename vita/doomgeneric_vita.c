/* doomgeneric_vita.c – Chex Quest / DOOM on PS Vita (fixed) */
...
/* === PALETTE: converte indici 8-bit → colori 32-bit Vita === */
...
/* Riempi schermo con un colore (per debug) */
...
/* ================================================================
   Blit: scala DG_ScreenBuffer (DOOM_W x DOOM_H, 32-bit)
         → Vita framebuffer (960 x 544)
   ================================================================ */
static void blit_to_screen(void) {
    ...
    step_x = (DOOM_W << 16) / VITA_W;
    step_y = (DOOM_H << 16) / VITA_H;
...
/* ================================================================
   DG interface (usato da doomgeneric.c)
   ================================================================ */
void DG_Init(void) {
    debug_log("DG_Init");
    base_time = get_ms();
...
void DG_DrawFrame(void) {
    /* Il rendering vero avviene in I_FinishUpdate().
       Qui facciamo solo il polling dell'input. */
    do_poll_input();
}
...
/* ================================================================
   I_* – Implementazioni per il motore Chocolate Doom
   ================================================================ */
void I_Init(void) {}
...
void I_WaitVBL(int count) {
    sceKernelDelayThread(count * 14286);
}
...
/* ================================================================
   VIDEO – Le 3 funzioni chiave (il cuore della fix)
   ================================================================ */
...
void I_InitGraphics(void) {
    ...
    /* Palette grayscale di fallback nel caso I_SetPalette
       non venga ancora chiamata */
    for (i = 0; i < 256; i++) {
...
void I_SetPalette(byte *doompalette) {
    /*  Doom ci passa 256 triplette R,G,B (768 byte).
        Convertiamo nel formato Vita A8B8G8R8:
          uint32_t  =  0xAA BB GG RR             */
    int i;
    for (i = 0; i < 256; i++) {
...
void I_FinishUpdate(void) {
    ...
    /* === PASSO 1: converti I_VideoBuffer (8-bit indici palette)
                    → DG_ScreenBuffer (32-bit colori Vita)       === */
    if (DG_ScreenBuffer) {
        for (i = 0; i < SCREENWIDTH * SCREENHEIGHT; i++) {
            DG_ScreenBuffer[i] = cmap[I_VideoBuffer[i]];
        }
    }

    /* === PASSO 2: scala e copia su framebuffer Vita === */
    blit_to_screen();

    if (frame_count < 10) {
        debug_logf("I_FinishUpdate %d: cmap[0]=%08X vid[0]=%u time=%d",
                   frame_count, cmap[0],
                   (unsigned)I_VideoBuffer[0], I_GetTime());
    }
    frame_count++;
}
...
int I_StartSound(void *s, int c, int v, int sep, int p) {
    (void)s; (void)c; (void)v; (void)sep; (void)p;
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    show_color(0xFF00FF00);    /* verde = display funziona */
